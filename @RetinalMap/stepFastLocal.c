...
mxArray *synapseNeighbourhood;
int *nSynapseNeighbourhood;
...
int getRandomNearbySC(int SCidx);
...
void mexFunction( int nlhs, mxArray *plhs[],
  ...
  if(nlhs != 6) {
    mexErrMsgTxt("stepFastLocal: Incorrect number of return arguments");
  }
  ...
  if(nrhs == 0) {
    mexErrMsgTxt("stepFastLocal: First argument object handle, second argument optional, nSteps");
  }
  
  if(!mxIsClass(prhs[0],"RetinalMap")) {
    mexErrMsgTxt("stepFastLocal: First argument should be a RetinalMap object");
  }
  ...
  synapseNeighbourhood = mxGetProperty(prhs[0],0,"synapseNeighbourhood");
  nSynapseNeighbourhood = mxGetData(mxGetProperty(prhs[0],0,
...
int getRandomNearbySC(int SCidx) {
  ...
  if(SCidx < 0) {
    mexPrintf("getRandomNearbySC: warning, called with negative SCidx\n");
    return -1;
  }

  if(nSC <= SCidx) {
    mexPrintf("getRandomNearbySC: SCidx %d out of range\n", SCidx);
    return -1;
  }

  mxArray *neigh = mxGetCell(synapseNeighbourhood, SCidx);

  if(nSynapseNeighbourhood[SCidx] == 0) {
    mexPrintf("SC neuron %d does not have any nearby SC\n", SCidx);
    return -1;
  }
  /* By doing floor we get indexes starting from 0 */
  int pos = (int) floor(nSynapseNeighbourhood[SCidx]*(rand()/((double) RAND_MAX + 1)));

  int *neighIdx = mxGetData(neigh);
...
int getRandomSCWeighted() {
  ...
  if(totalSynapsesInSystem == 0) {
    mexPrintf("getRandomSCWeighted: No synapses in simulation\n");
    /* No synapses in the system */
    return -1;
  }
...
double calculateSynapseAdditionEnergy(int SCidx, int RGCidx) {
  ...
  case 3: /* Servomechanism */
    ...
    break;
  default:
    mexErrMsgTxt("stepFastLocal: Unknown type flag!");
    Echem = 0;
    break;
  ...
  Eact = 0.0;
	...
	if(gammaAct > 0) {
		...
		for(j = 0; j < nSelfCon; j++) {
			...
			if(RGCidx == selfCon) {
				/* The synapse we are adding should be included 
					 ...
					 included n in the previous loop, so just add n+1 */
		...
		tmpCW += CAct[0]*w; /* C = 1, for self-comparisons normally!! */
...
void step(int nSteps) {
  ...
  mexPrintf("Running %d steps (local jumps, even sampling of synapses)\n", 
	    nSteps);
  ...
  for(i = 0; i < nSteps; i++) {
    ...
    /* Pick a post synaptic SC randomly, but weighted by number of synapses */
    SCidx = getRandomSCWeighted();

    /* Find an existing presynaptic RGC on that SC neuron */
    RGCidx = getRandomExistingConnection(SCidx);

    /* Find a SC neuron nearby the post synaptic SC neuron */
    SCidx = getRandomNearbySC(SCidx);

    /* Try to add a synapse between the RGC and the neighbour SC */
...
void testAddRemovalSymmetry() {
  ...
  mexPrintf("Testing add/removal symmetry\n");
  ...
  if(SCidx < 0 || RGCidx < 0) {
    mexErrMsgTxt("Unable to place a test synapse after 100 trials, aborting.\n");
  }
